...
impl Contract {
    ...
    async fn create_pair(
        ...
        // let (pair_actor, result): (_, Result<(), dex_io::Error>) =
        ...
        let create_res = ProgramGenerator::create_program(
            self.pair,
            dex_io::Initialize {
                pair: token_pair,
                factory: exec::program_id(),
        ...
        let Ok(future) = create_res else {
            return Err(Error::GstdError("Failed to create program".to_string()));
        };
        let (message_id, pair_actor) = future;
...
async fn process_handle() -> Result<Event, Error> {
    ...
    match action {
        ...
        Action::CreatePair(token_a, token_b, router) => {
            contract.create_pair(token_a, token_b, router).await
        }
        Action::GetFeeTo => Ok(Event::FeeToSet(contract.fee_to)),
        Action::GetPair(token_a, token_b) => {
            let token_pair = if token_b > token_a {
                (token_b, token_a)
            } else {
                (token_a, token_b)
            };
            if let Some(pair) = contract.pairs.get(&token_pair) {
                Ok(Event::Pair(*pair))
            } else {
                Err(Error::GstdError("Pair not found".to_string()))
            }
        }
    }
}
